...
* added filter (license) argument to `get_uuid()`, `pick_phylopic()`, `add_phylopic_base()`, `add_phylopic()`, and `geom_phylopic()` (#72)
...
* fixed the behavior of `geom_phylopic()` when used with `ggplot2::coord_sf()`
