...
Herein we provide three example applications of the **rphylopic** package using base R. However, note that all demonstrated functionality is also available for use with the `{ggplot2}` package and showcased in a [separate vignette](2-advanced-ggplot2.html).
...
for (i in seq_along(species_split)) {
  ...
  plot(x = species_data$bill_length_mm, y = species_data$flipper_length_mm,
       ...
       xlim = range(penguins_subset$bill_length_mm, na.rm = T),
       ylim = range(penguins_subset$flipper_length_mm, na.rm = T))
}
```
...
for (i in seq_along(species_split)) {
  ...
  add_phylopic_base(img = penguin_rot,
                    x = species_data$bill_length_mm, y = species_data$flipper_length_mm)
}
```
...
```{r echo = FALSE}
vertebrate.data$uuid[vertebrate.data$species == "Sus_scrofa"] <- "87047da1-b40e-4b31-8492-4db262f129f5"
```
...
```{r}
vertebrate.data$svg <- lapply(vertebrate.data$uuid, get_phylopic)
```
...
axis_geo_phylo(intervals = "periods")
add_phylopic_base(img = vertebrate.data$svg,
...
```{r}
vertebrate.data$svg[[1]] <- rotate_phylopic(vertebrate.data$svg[[1]])
vertebrate.data$svg[[8]] <- rotate_phylopic(vertebrate.data$svg[[8]])
```
