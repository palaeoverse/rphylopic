...
GeomPhylopic <- ggproto(
  ...
  setup_data = function(data, params) {
    ...
    if (is.list(params$filter)) {
      params$filter <- params$filter[[1]]
    }
    # Check that only one silhouette aesthetic exists
    data_cols <- sapply(c("img", "name", "uuid"), function(col) {
      col %in% colnames(data)
    })
    param_cols <- sapply(c("img", "name", "uuid"), function(col) {
      col %in% names(params)
    })
    cols <- data_cols | param_cols
    if (sum(cols) != 1) {
      stop(paste(
        "Must specify one (and only one) of the `img`, `name`, or",
        "`uuid` aesthetics."
      ))
    }
    ...
    if (cols["name"]) {
      if (!params$verbose) {
        warning(
          paste(
            "You've used the `name` aesthetic/argument. You may want",
      ...
      imgs <- sapply(name_unique, function(name) {
        uuid <- tryCatch(
          get_uuid(name = name, filter = params$filter),
      ...
      imgs <- sapply(uuid_unique, function(uuid) {
        img <- tryCatch(get_phylopic(uuid), error = function(cond) NULL)
        if (is.null(img)) {
          warning(
            paste0('"', uuid, '"', " is not a valid PhyloPic `uuid`."),
            call. = FALSE
          )
        }
        img
      ...
      if (
        any(sapply(imgs, function(img) {
          !is(img, "Picture") && !is.array(img)
        }))
      ) {
        stop(paste(
          "The `img` aesthetic should be of class Picture (for a",
...
phylopic_key_glyph <- function(img = NULL, name = NULL, uuid = NULL) {
  ...
  if (!is.null(name)) {
    ...
    imgs <- sapply(name_unique, function(x) {
      url <- tryCatch(get_uuid(name = x, url = TRUE), error = function(cond) NA)
      if (is.na(url)) {
        warning(paste0(
          "`name` ",
          '"',
...
#' @rdname scales
scale_height_continuous <- function(
  name = waiver(),
  breaks = waiver(),
  labels = waiver(),
  limits = NULL,
...
#' @importFrom scales pal_area
#' @rdname scales
scale_width_continuous <- function(
  name = waiver(),
  breaks = waiver(),
  labels = waiver(),
  limits = NULL,
