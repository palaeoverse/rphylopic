...
resolve_phylopic <- function(
  ...
  if (api == "eol.org") {
    ...
    if (hierarchy) {
      url <- paste0(
        "https://api.gbif.org/v1/species/match?verbose=true&",
        "name=",
        URLencode(jsn$canonicalName[1])
      )
      jsn <- json_GET(url)
      ids <- c(
        jsn$speciesKey[1],
    ...
    namespace <- "txn"
    url <- paste0(
      "https://paleobiodb.org/data1.2/taxa/auto.json?",
      "limit=10&name=",
      name_encode
    )
    jsn <- json_GET(url)
  ...
  for (i in seq_along(ids)) {
    api_return <- phy_GET(
      path = paste0("resolve/", api, "/", namespace, "/", ids[i])
    )
