test_that("flip_phylopic works", {
  ...
  expect_equal(
    diff(cat@summary@xscale),
    diff(flip_phylopic(cat, horizontal = TRUE)@summary@xscale)
  )
  expect_equal(
    diff(cat@summary@yscale),
    diff(flip_phylopic(cat, horizontal = FALSE, vertical = TRUE)@summary@yscale)
  )
...
test_that("rotate_phylopic works", {
  ...
  expect_true(all.equal(
    rotate_phylopic(cat)@summary@yscale,
    rev(cat@summary@xscale)
  ))
  expect_true(all.equal(
    rotate_phylopic(cat, 180)@summary@xscale,
...
test_that("recolor_phylopic works", {
  ...
  expect_equal(
    dim(recolor_phylopic(cat_png[,, 1, drop = FALSE], .5, fill = "red")),
    dim(cat_recolor)
  )
  expect_equal(
    dim(recolor_phylopic(cat_recolor[,, 1:3, drop = FALSE], .2)),
    dim(cat_recolor)
  )
...
test_that("array methods work", {
  skip_if_offline(host = "api.phylopic.org")
  cat <- get_phylopic("23cd6aa4-9587-4a2e-8e26-de42885004c9", format = "raster")
