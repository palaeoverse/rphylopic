test_that("get_phylopic cache makes second call faster", {
  ...
  svg_key <- sprintf("https://images.phylopic.org/images/%s/vector.svg", id)
...
test_that("get_phylopic raster cache makes second call faster", {
  ...
  png_key <- sprintf("https://images.phylopic.org/images/%s/vector.svg", id)
  ...
  t1 <- system.time(
    jay1 <- get_phylopic(id, format = "raster", height = height)
  )[["elapsed"]]
...
test_that("get_phylopic raster cache is keyed per height", {
  ...
  png_key_height <- function(h) {
    sprintf("https://images.phylopic.org/images/%s/vector.svg?h=%s", id, h)
  }
  ...
  jay512 <- get_phylopic(id, format = "raster", height = 512)
  expect_true(httpcache::hitCache(png_key))
