test_that("resolve_phylopic works with GBIF", {
  ...
  expect_error(resolve_phylopic(name = "fdsafdsafdas;fdsaf", api = "gbif.org"))
  expect_error(resolve_phylopic(
    name = "Acropora cervicornis",
    api = "gbif.org",
    n = "hi"
  ))
...
test_that("resolve_phylopic works with EOL", {
  ...
  suppressWarnings(
    res <- resolve_phylopic(name = "Enhydra lutris", api = "eol.org", n = 5)
  )
  expect_equal(length(res), 1)
  expect_true(is.character(names(res)))
  expect_true(all(sapply(res, is.character)))
  ...
  expect_error(resolve_phylopic(name = "fdsafdsafdas;fdsaf", api = "eol.org"))
})
...
test_that("resolve_phylopic works with PBDB", {
  ...
  tryCatch(check_url("https://paleobiodb.org/data1.2/"), error = function(e) {
    skip()
  })
  skip_if_offline(host = "api.phylopic.org")
  res <- resolve_phylopic(
