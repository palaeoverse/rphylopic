...
test_that("geom_phylopic works", {
  ...
  df <- data.frame(
    ...
    y = seq(10, 25, 5),
    uuid = c(
      "e25f1863-331b-4891-8084-fe8602e4cf8d",
  ...
  gg <- ggplot(df) +
    geom_phylopic(
      aes(x = x, y = y, uuid = uuid),
      height = c(NA, 10, NA, 10),
      width = c(0.5, NA, 0.5, NA)
    ) +
    coord_cartesian(xlim = c(1, 6), ylim = c(5, 30)) +
    theme_classic(base_size = 16)
...
test_that("phylopic_key_glyph works", {
  ...
  gg <- ggplot(df) +
    geom_phylopic(
      aes(x = x, y = y, name = name, color = name),
      height = 10,
      show.legend = TRUE,
      verbose = TRUE,
  ...
  expect_error(
    ggplot(df) +
      geom_phylopic(
        ...
        show.legend = TRUE,
        verbose = TRUE,
        key_glyph = phylopic_key_glyph(name = 12345)
      ) +
      coord_cartesian(xlim = c(1, 6), ylim = c(5, 30)) +
      theme_classic(base_size = 16)
  )
  ...
  expect_error(
    ggplot(df) +
      geom_phylopic(
        ...
        show.legend = TRUE,
        verbose = TRUE,
        key_glyph = phylopic_key_glyph(uuid = 12345)
      ) +
      coord_cartesian(xlim = c(1, 6), ylim = c(5, 30)) +
      theme_classic(base_size = 16)
  )
  ...
  expect_error(
    ggplot(df) +
      geom_phylopic(
        ...
        show.legend = TRUE,
        verbose = TRUE,
        key_glyph = phylopic_key_glyph(img = 12345)
      ) +
      coord_cartesian(xlim = c(1, 6), ylim = c(5, 30)) +
      theme_classic(base_size = 16)
  )
